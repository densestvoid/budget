-- View that dynamically matches recurring transactions to actual transactions
-- Matches the first transaction that occurs on or after the expected date and before the next expected date
-- Handles weekly, monthly, and yearly recurrences based on start_date, recurrence_unit, and recurrence_value
-- Supports both expenses (negative amounts) and income (positive amounts)
-- Ensures: (1) one transaction per recurring transaction per occurrence period, (2) each transaction matches at most one recurring transaction
